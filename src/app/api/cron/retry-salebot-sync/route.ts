/**
 * Крон `retry-salebot-sync` — Чертёж.md, БЛОК 5 «Cron-задачи»: по спецификации
 * каждые 15 мин, СЕЙЧАС временно раз в сутки (`vercel.json`, 01:00 MSK) — на
 * бесплатном тарифе Vercel (Hobby) крон-задачи чаще одного раза в сутки не
 * разрешены. Логика ниже от частоты не зависит (просто реже обнаруживает
 * рассинхрон), вернуть на 15 мин — правка одной строки в `vercel.json` при
 * переходе на Pro.
